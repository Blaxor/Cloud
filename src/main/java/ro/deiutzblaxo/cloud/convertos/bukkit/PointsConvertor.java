...
public class PointsConvertor {
    ...
    public static Point2D locationToPoint2D(Location location) {
        return new Point2D((float) location.getX(), (float) location.getZ());
    }

    public static Point3D locationToPoint3D(Location location) {
        return new Point3D((float) location.getX(), (float) location.getY(), (float) location.getZ());
    }

    public static Location point2DToLocation(Point2D point2D, World world, double y) {
        return new Location(world, point2D.X, y, point2D.Y);
    }

    public static Location point3DToLocation(Point3D point3D, World world) {
